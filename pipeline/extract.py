...
SKIP_HREF_PAT = re.compile(
    ...
    r"writers?|ty-writer|authors?|products?|personal_zone|home\.aspx)(/|$)"
    r"|\.tag$",
    re.I,
)
...
def extract_items(html: str, base_url: str, selector: str = None, lang: str = "he"):
    ...
    for a in scope.find_all("a", href=True):
        ...
        if SKIP_HREF_PAT.search(pu.path):
            continue
        # Links to the site root or to a digit-less single-segment path are
        # section/nav destinations (homepage self-links, "skip to content",
        # ynet-style section indexes), never articles.
        segs = [s for s in pu.path.split("/") if s]
        if not segs or (len(segs) == 1 and not any(c.isdigit() for c in segs[0])):
            continue
        key = text.lower()
